...
distributions, such that they become uncorrrelated, see \equref{source_trans} and \equref{target_trans}
respectivelely. Subsequently the transformation chain that has been applied to the target distributions
are inverted and then applied to the source ones, \equref{match_distributions}. The three transforamtions
...
Both of the above theorems can only be directly aplied to continious distributions.
However, exact analytical shape of the \pdfs or CDFs involved is not required. This is bacause the CDF
of any variable $x$ can be built by essentiall computing the integral-sum of \equref{cdf_def}. In order
...
of the previous transformations are direct implementaions of \theoref{theo_inv_trans_sampling}.
...
Comming now to the last transformation step, \gGausUn{}, of \equref{hor_rew_logic}.
...
\noindent The covariance between varibles is computed using the standard formula of \equref{hor_reww_correlation_formula}
...
given distribution, $x$, and ascociating the a value of the integral of \equref{cdf_def}, name it $c_i$,
...
continues is using the well known techinique of {\it linear interpolation}, shown in \figref{linear_interpolation},
