...
that a given source or target bin has zero entries for a given binning scheme. This situation gets
...
to matching distributions is meant to bypass the problem of binning, especially in many dimensions,
...
involved in the current approach: Transformation $(A)$ converts the input distribution to flat.
...
appendix is to quickly demonstrate the advertised technique. However there is plethora of examples
and profs that can be found online starting from {\it Wikipedia} for example}.
...
\begin{theorem}[Probability Integral Transform]
  ...
  Then the variable $u=F_x(x)$ is uniform. This theorem is inverse of \theoref{theo_inv_trans_sampling}.
...
is exhausted}. It is important to point out that this binning can be arbitrarily fine\footnote{However from
implementation point of you the larger the number of bins the slower the algorithm performs. Here is where
the built-in function {\tt numpy.digitise()} of {\tt python} proves to be useful, as the timing scales
nicely with the number of bins.} without having the problems explained in the introduction, since it is not
...
the {\it inverse error function} and the {\it complementary error function} respectively, which
...
To illustrate that the advertised technique works a typical problem in high energy physics is addressed.
...
monte carlo (source) and background subtracted data (target), see the top two plots of \figref{hor_rew_example_figs}.
Furthermore, the previous distributions are correlated and combined with a fine binning will yield problems as explained
earlier in the vertical weighting subsection. After applying the technique advertised before the
$\parenthesis{\ptot(\kaon)-\ptot(\pion)}$ distributions statistically compatible, bypassing the risks associated
