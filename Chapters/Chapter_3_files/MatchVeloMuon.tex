...
At least 3 hits in the muon stations M2-M5 are required by the \mvm algorithm in order to accept the initial \veloTrack.
...
area of the magnetic field, see \figref{det_evt_display}. However estimating this trajectory consumes valuable computation
...
can be seen in \figref{mvm_eff_p_comp}, where the improvement of the upgraded \mvTTm algorithm are significant.
...
As explained in \secref{sec:muon_matching} the M3 FoI $x$ coordinates are computed assuming a minimum track momentum, and thus maximum deflection,
...
Which implies that most of the times the true hit is inside the FoI. The outliers in the \figref{mvTTm_res_x} in with $|d^{M3}|>3\m$
