...
The \mvm algorithm takes a \veloTrack as input and checks whether it is compatible with being a muon by
...
M3 is the first station to search for hits for two reasons: First the number of hits in M2 is large compared
to M3, see Figure 3.2 in \cite{roelThesis}; Thus an initial \veloTrack candidate would result in a large number
of seed hits, which is not computationally feasible. Second the muon reconstruction algorithm (\isMuon) will
always fail if there is no M3 muon hit assigned to a \veloTrack.

Coming now to the issue of the FoI size computation. As it can be seen in \figref{lhcb_detector_cross_section},
...
At least three hits in the muon stations M2-M5 are required by the \mvm algorithm in order to accept the initial \veloTrack.
...
as explained earlier in \secref{sec:muon_matching}. Second, the parametrization of the magnet $z$
...
can be seen in \figref{mvm_eff_p_comp}, where the improvement of the upgraded \mvTTm algorithm is shown to be significant.
...
in \secref{muid_hlt1}, the high $\pt$ muons will not be processed by the \mvTTm algorithm and thus
...
of the $\chisq/\nDoF$ discriminating power between the old and upgraded algorithm is shown in \figref{mvm_chi2_comp}.
...
\begin{figure}[t]
  ...
  \begin{subfigure}{0.5\textwidth}
    ...
    \scalebox{0.6}{\input{Figures/Chapter3/cdf_sig_bkg_mvm}}
    \caption{}
    \label{mvm_cdf}
  \end{subfigure}%
  \hfill%
  \begin{subfigure}{0.5\textwidth}
    ...
    \scalebox{0.6}{\input{Figures/Chapter3/roc_curve_mvm}}
    \caption{}
    \label{mvm_roc}
  \end{subfigure}
...
Based on the signal and background distributions of \figref{mvTTm_chi2} and \figref{mvm_cdf}, the last value implies that;
...
mentioned in \secref{muid_hlt1}. The last FoIs are momentum dependant search windows, implying
