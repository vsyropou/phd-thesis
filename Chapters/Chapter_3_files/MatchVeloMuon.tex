...
Reagarding the issue of FoI size computation: As it can be seen in \figref{lhcb_detector_cross_section},
...
This is achieved by reducing fake tracks also known as {\it ghost tracks}.
...
is estimated using simulated \Sigmapmumu events. The muons from this decay are required to pass the full 
track reconstruction and be positivelly identified as muons. The latter is achieved by exploiting the fact 
that the true muon track is know when using simulated events. These two conditions define the {\it true muons}.
Subsequently, given and intial \veloTracks


 versus the total and transverse momentum and compared against the old algorithm. A common cutoff value for
...
\begin{figure}[t]
  ...
  \begin{subfigure}{0.5\textwidth}
    ...
    \scalebox{0.6}{\input{Figures/Chapter3/MvTTM/cdf_sig_bkg_mvm}}
    \caption{}
    \label{mvm_cdf}
  \end{subfigure}%
  \hfill%
  \begin{subfigure}{0.5\textwidth}
    ...
    \scalebox{0.6}{\input{Figures/Chapter3/MvTTM/roc_curve_mvm}}
    \caption{}
    \label{mvm_roc}
  \end{subfigure}
