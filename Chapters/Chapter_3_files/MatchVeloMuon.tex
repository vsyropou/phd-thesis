...
M3 is the first station to search for hits for two reasons. First the occupancy{\color{red} define} in M2 is very high,
see Figure 3.2 in \cite{roelThesis}, thus an initial a single VELO track candidate would create
...
These FoIs account for the fact that the presence of heavy material between the muon stations affects the
trajectory of a muon passing thorough, this effect is called multiple scattering, and it is more pronounced for low momentum tracks.
...
for each candidate muon track using the kick method \cite{Hommels:999327}.
...
\veloTracks with \ttracker information increases the purity\footnote{by reducing the number a certain type of fake
tracks called {\it ghost tracks}}\cite{Bowen:2105078}.
