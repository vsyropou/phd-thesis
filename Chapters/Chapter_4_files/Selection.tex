...
The \BsJpsiKst signal yield out of the full 3 \invfb data-set is expected to be low, see \secref{TheBsJpsiKstDecay}. Thus, one would like to
...
For example disentangle signal from background. Further details in \secref{sWeighting_and_mass}. }.
...
near the \BJpsiKpi signal peaks. Furthermore given the low number of expected peaking background events, see \tabref{peaking_bkg_yields},
their distribution is rather discontinuous with sharp spikes. This type of distributions have to be removed
before the next selection step described in \secref{sWeighting_and_mass} is applied. The last technique is more effective
