...
candidate during the event reconstruction, see \secref{reconstruction}.
...
whereas background candidates near -1, see \figref{BTDG_performance}. In the current analysis the calsifyer variable is aslo refered
...
For the signal representative samples, \BsJpsiKst Monte-Carlo simulated data are used, here after simulated data.
As for the background representative sample,
...
\noindent real data candidates are used. These candidates are taken from a control
...
The two well seperated peaks in \figref{BTDG_performance} suggest that the BDT variable, shows a good discrimination
...
see \tabref{peaking_bkg_yields}, their distribution is quite non-uniform and discontinues.
...
The estiamted yields of non-combinatorial background after the above mentioned steps is shown in \tabref{peaking_bkg_yields}
