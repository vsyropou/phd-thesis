...
With the exception of the trigger, addressed in \secref{det_trigger}, there are effectively four stages of offline event
...
tool to unfold data distributions \cite{splot}, for example signal and background. A \pdf that is able to 
...
of \equref{eqn:fom} essentially expresses the signal fraction. For a range of BDT values the \sPlot technique
...
enough, see \figref{mass_plot}, and the \sPlot technique can be used to remove this specific reflection
background.
...
First, a hyperbolic shape core aims at describing the peak of the previous distributions. Second, invariant mass resolution effects are
taken into account by marginalizing over the \Bq invariant mass uncertainty. Overall the Hypatia function adequately
...
the subsequent angular fit, the requirements of the \sPlot technique are not satisfied\footnote{as explained in the beginning
...
\secref{Toy_Experiments_Study} where a toy study is performed for the angular fitting model of \secref{Total_Decay_Rate}.
