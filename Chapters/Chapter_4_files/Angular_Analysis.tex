...
\begin{table}[h]
  \centering 
  \caption{ Efficiency moments. {\color{red} comment on the values} }
  \renewcommand{\arraystretch}{1.2}
...
parameters and the two dimensional $p_{K^{\pm}},p_{\pi^{\mp}}$ ({\color{red} why not the muons)} momentum distribution. 
...
with angular momentum barier factors. Whereas a Isobar is a superposition of resonances. ({\color{red} Make sure you understand what you say here and find references or show formulas.})
...
\tabref{}, where the \CSP factors are shown for each \mkpi bin {\color{red} Do this table, its easy}. The acceptance is also split
...
the onnection between the \pdf and the likelihood estimation of the parameters of interest. Thus, starting from
...
useful\footnote{and perhaps beautyfull} to realise that by maximizing the likelihood function one gets the best estimate
for the parameters of intereset given the data based on the \pdf. This is done by means of the estimator [ref]... blahh{\color{red} need more studing for that.}
...
The estimation of the best fit values and their correponding correlations are computed based on  blhaa \equref{}.

Coming now to the concept of an extended likelihood.
\begin{equation}
-\text{log}{L} = \sum_e^n \mathcal{P}(\vec{x};\vec{p}) - P(v(\vec{p});y)
% \Rightarrow -\log{L} = \sum_e^n \mathcal{P}(\vec{x};\vec{p})
\label{extlikelihood}
\end{equation}

This way the measured number of events becomes a free parameter that can be fitted for. 
THis will be exploited later since after all the analysis
