...
The acronym P2VV acronym refers to the spin of the particles involved in the decay. Specifically, the
\Bs (and \Bd) is spin-0 parity minus (Pseudo-scalar) particle, whereas the intermediate resonances
...
% In the transversity framework the decay
% amplitude is decomposed to the three possible polarization states of the intermediate particles \jpsi and \Kst with respect to
% the \Bs rest frame using polarization vectors of the decay amplitude
...
Here, the steps to arrive at an expression for the angular decay rate for \BsJpsiKst decays. In \equref{ang_terms}
the amplitude $\mathcal{A}(\text{\BJpsiKst})$ is decomposed in its \pwave ($0$, $\parallel$, $\perp$) and \swave ($S$)
polarizations.
...
\noindent where the terms $a_n$ orginate from squaring the amplitude and their exact expressions are shown in the first column of
...
Note the appearance of \pwave and \spwave interference terms. The functions $h_n$ represent the angular dependence of each term,
and $\mKpiAmp$ the \mkpi dependence of the amplitude. The treatment of the \mkpi dependence is postponed for the next section.
\tabref{ang_distr} lists the angular part of the decay rate equation ({\it i.e.} the $h_n$ functions) which is the result of
...
Lastly, the orthogonal angular basis of $P$ and $Y$
...
of the angular acceptance, see see \secref{Accceptance}.

% $\int_{-1}^{1}Pd\cos\thetaK$ and $\int_{\cos\theta=-1,\varphi=-\pi}^{\cos\theta=1,\varphi=\pi}Yd\cos\theta d\varphi$
...
on the other hand, sources introducing angular acceptance need to be acounted for. Diviations from a perfect
angular acceptance can be introduced by selection criteria imposed during the Stripping or by the detector itself.
...
which when combined with the angular description formalism in the previous subsection, allows fort fast aalytical
...
The efficiency moments used to describe the acceptance
...
This projection is shown in \equref{eff_moms_def}. The factor $j+\frac{1}{2}$ is used to account for the fact that the associated Legendre
...
\begin{table}[h]
  ...
  \caption{ Efficiency moments of \BsJpsiKst. The larger the value of a moment, the more the corresponding function contributes to the overall efficiency function.
            ...
            $\cos\thetaK$ projection. Acceptanceeffecets in the $\cos\thetamu$ projection are less pronounced compared to the one $\cos\thetaK$. This can be deduced from
            ...
            the constraint are explained in the current subsection. The efficiency moments are correlated and the corresponding correlation matrix can be found
...
which is not a problem by itself. However, the parametrization of the acceptance function, \equref{eff_func} may become negative.
...
acceptance function, \equref{eff_func}, is introduced to force it to be positiv definite and second, a fit to the efficiency
...
Having chosen a set of moments the efficiency function has to be constrained such that the
condition of \equref{eff_func_constr} is satisfied.
...
\noindent It turns out that this condition can be easilisy satisfied if the basis that the
...
\noindent resulting in the following efficiency function of \equref{eff_func_new}.
...
The result of the above constraint is shown in figure \figref{angAcc_constr_fit}, where it can be seen that the acceptance projection on $\cos\thetaK=1$
...
\begin{tabular}{ c c c c c c c c c }
  ...
  \hline
  $\fP{0}$                            &  -1.15  &  +0.96  &  +0.19  &  +0.0   &  -0.02  &  -0.01   \\
  $\fP{\parallel}$                    &  -0.05  &  -0.06  &  +0.03  &  +0.04  &  +0.02  &  +0.01   \\
  $\ampPhase{\parallel}$              &  -0.37  &  +0.23  &  +0.09  &  +0.03  &  +0.01  &  +0.0    \\
  $\ampPhase{\perp}$                  &  -0.57  &  +0.28  &  +0.16  &  +0.07  &  +0.03  &  +0.01   \\
  \hline
...
\caption{Parameters of interest change after each iteration. The amount of change is expresed in units of one standard deviation.
         ...
         parameters of interest converge.}
...
acceptance corrections is small. \figref{angAggCorrections} shows (in arbitrary units) the shape of the acceptance before and after corrections.
This is also supported by the effect of these corrections on the paramters of intereset which are shown in \tabref{phys_pars_change_iter}.
...
\begin{figure}[h]
  ...
  \caption{Angular acceptance shape from simulated data before(black points) and after(red points) acceptance corrections.
           The plots are made in the same way as in \figref{angAcc_all}.}
...
\begin{tabular}{c c c}
 \hline
 parameter   &     difference   &  difference / $\sigma$ \\
 \hline
$\Acp{0}$                & -0.0000  & -0.00   \\
$\Acp{\text{S}}$         & -0.0008  & -0.01   \\
$\Acp{\parallel}$        & +0.0196  & +0.13   \\
$\Acp{\perp}$            & -0.0190  & -0.19   \\
\hline
$\fP{0}$                 & -0.0276  & -1.14   \\
$\fP{\parallel}$         & -0.0014  & -0.05   \\
$\ampPhase{\parallel}$   & -0.0583  & -0.37   \\
$\ampPhase{\perp}$       & -0.0640  & -0.57   \\
\hline
$\fS{1}$                 & +0.0975  & +0.87   \\
$\fS{2}$                 & +0.0209  & +0.74   \\
$\fS{3}$                 & +0.0001  & +0.00   \\
$\fS{4}$                 & +0.0525  & +0.48   \\
$\deltaS{1}$             & -0.3205  & -2.09   \\
$\deltaS{2}$             & +0.0283  & +0.12   \\
$\deltaS{3}$             & -0.1866  & -1.02   \\
$\deltaS{4}$             & -0.3165  & -2.39   \\
    \hline
 \end{tabular}
 \caption{Differences in the parameters of intereset before and after corrections.}
 \label{phys_pars_change_iter}
\end{table}
...
It originates from integrals like the one in equation \equref{spIntegrals}
...
\noindent where $\mKpiAmp[n,m]$ run in the same way as in \equref{ang_terms}, whilst $\Phi$ is a phase space
factor shown in \equref{csp_ph_sp_factor}
...
terms\footnote{The diagonal $m=n$ integrals are by construction equal to 1.} of \tabref{eff_moms_table} as shown in \equref{csp_def}.
...
Indeed, the \Bs \mkpi \sPlot seems to be slightly distorted, \figref{mkpiPlot_raw}. Since there is no \aprior reason why the \Kst$~$\pwave
...
in \equref{eff_func_new}. This way the interference between the $K\pi$ \swave and the \Kstarz $ $ \pwave vanishes, since
...
\begin{figure}[h]
  ...
  \begin{subfigure}{0.5\textwidth}
    ...
    \scalebox{0.65}{\input{Figures/Chapter4/eff_corrected_raw_mdau2}}
    \caption{}
    \label{mkpiPlot_raw}
  \end{subfigure}%
  \hfill%
  \begin{subfigure}{0.5\textwidth}
    ...
    \scalebox{0.65}{\input{Figures/Chapter4/eff_corrected_eff_mdau2}}
    \caption{}
    \label{mkpiPlot_eff}
  \end{subfigure}
  \begin{subfigure}{0.5\textwidth}
    \tikzsetnextfilename{eff_corrected_raw_mdau1}
    \scalebox{0.65}{\input{Figures/Chapter4/eff_corrected_raw_mdau1}}
    \caption{}
    \label{jpsiPlot}
  \end{subfigure}
...
the parameters of intereset, $\Acp{}$, introduced in \secref{cp_assymetries_and_total_decay_rate}.
...
The quantities $\Delta\Gamma_s$ and $\Delta m_s$ have been described in \secref{Phenomenology},
...
more details in \secref{cp_assymetries_and_total_decay_rate}.}. This significantly simplifyies the above computation.
...
found to vary significantly different between each \mkpi interval, see \tabref{moms_final_neg} and \tabref{moms_final_pos}. The chosen \mkpi binning
...
\noindent Note that constant terms in the likelihood affect neither the shape of the likelihood nor the position of the minimum
