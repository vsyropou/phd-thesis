...
\begin{figure}[h]
  ...
  \begin{subfigure}{0.49\textwidth}
    ...
    \scalebox{1.3}{\input{Figures/Chapter4/eff_data_helcosthetaK_allKaons_binall}}
    \caption{}
    \label{angAcc_ctk}
  \end{subfigure}%
  \hfill%
  \begin{subfigure}{0.49\textwidth}
    ...
    \scalebox{1.3}{\input{Figures/Chapter4/eff_data_helcosthetaL_allKaons_binall}}
    \caption{}
    \label{angAcc_ctl}
  \end{subfigure}
  ...
  \begin{subfigure}{0.49\textwidth}
    ...
    \scalebox{1.3}{\input{Figures/Chapter4/eff_data_helphi_allKaons_binall}}
    \caption{}
    \label{angAcc_phi}
  \end{subfigure}
...
Before laying out the formulas behind the above mentioned steps it is usefull to literally see the problem caused by the parametrization, see \figref{angAcc_nom}
...
\noindent The result of the above constrain is shown in figure \figref{angAcc_constr_fit}, where it can be seen that the acceptance projecton on $\cos\thetaK=1$
...
Constraining the acceptance in this maner described above comes with a small penalty, meaning that the shape of the 1D acceptance
...
means of a multivariant gaussian constrain. The fitting \pdf is shown in \equref{mvm_constr} and it consists of the \pdf that the simulated 
...
a systematic ucertainty, see \secref{systAngAcc}. The last two facts makes, in hindshight, the acceptance correction procedure not necessary. 
...
is modeled directly via the $\mKpiAmp$ terms of \equref{ang_terms}. The above mentioned \swave lineshapes that are necessary
...
one gets the production assymetry estimate shown in \equref{prod_assym}.
...
\BsJpsiKst. As it was explained in \secref{TheBsJpsiKstDecay} those decays are flavour specific which means that a \Bs messon
...
more details in \secref{cp_assymetries_and_total_decay_rate}.} and thus significantly simplifing the above computation.
...
In addition the \CSP factors of \secref{Kpi_Invariant_mass} need to be calculated and inserted seperatelly for each component \pdf resulting in
...
\noindent Another typical way to estimate and better visualise the standard diviations and covvariances is shown in \secref{nllscans}. 
...
to vary $\Acp{\text{raw}}$ in the fit it is necessary to reparametrize them in a different way shown in \equref{acp_param}.
...
\noindent The $\AmpPlSq{i}$ and $\AmpMnSq{i}$ are the amplitude expresions that are used along with the angular functions of \tabref{ang_terms}.
...
The fractions $\fP{k}$ and $\fS{}$ are the actuall parameters that one wish to measure and correspond to those presented in \secref{TheBsJpsiKstDecay}.
...
assymetries were estimated, the observed yields are made dependent on the \ACP assymetry in \equref{acp_yields} 
