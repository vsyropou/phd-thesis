...
orginate from squaring the amplitude and their exact expressions are shown in the first column of \tabref{ang_distr}.
...
same steps selection as the real data) as shown in \equref{eff_moms_calc}. Acceptance effecets in the $\cos\thetamu$ projection are less
pronounced compared to the one $\cos\thetaK$. This can be deduced from \figref{angAcc_ctl} as well as from the values of moments like
...
correlation matrix can be found in \tabref{eff_moms_corr}
...
The $p_{K^{\pm}},p_{\pi^{\mp}}$ weighting of the MC sample is done using 2D histograms, following the variable intervals defined in \tabref{angAccBinning}.
