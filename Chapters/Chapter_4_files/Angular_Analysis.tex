...
can be found in \cite{daanThesis} and \cite{jeroenThesis} respectively for the transversity and helicity formalism.
...
\noindent Note that the \swave is not a \PtoVV process since no intermediate \Kstarz resonance is produced.
 The terms $a_n$ originate from squaring the amplitude and their exact expressions are shown in \tabref{ang_distr}.
...
spectrum is harder and the $(\ptrans{\kaon}-\cos\thetaK)$ is more symmetric with respect to $\cos\thetaK$, see \figref{ctk_hadrons_pt}. Given these
...
Thus, \equref{pi_angle_massless} implies that decays where $\cos\thetapi=-1$ are not
...
Reagarding the angular acceptance parametrization, there are at least two ways to proceed,
...
since the sum in \equref{eff_func} runs through all allowed values of the $ijk$ indices. However, in practice only a small number of efficiency moments
...
This implies that the integral in \equref{eff_moms_def} can be estimated by a sum over the simulated data
...
(see \tabref{eff_moms_table} ) to counter the large negative value of $\moment{1}{0}{0}$ but unfortunately it is not enough.
...
of \equref{eff_func}, is modified as follows:
...
\begin{figure}[t]
  ...
  \begin{subfigure}{0.5\textwidth}
    \raggedright
    \tikzsetnextfilename{canv_cosThK_cosThL_Sim08_3fb_hel_negKaons_all}
    \scalebox{0.55}{\input{Figures/Chapter4/canv_cosThK_cosThL_Sim08_3fb_hel_negKaons_all}}
    \caption{}
    \label{eff2D_kl}
  \end{subfigure}%
  \hfill%
  \begin{subfigure}{0.5\textwidth}
    \raggedleft
    \tikzsetnextfilename{canv_cosThK_phi_Sim08_3fb_hel_negKaons_all}
    \scalebox{0.55}{\input{Figures/Chapter4/canv_cosThK_phi_Sim08_3fb_hel_negKaons_all}}
    \caption{}
    \label{eff2D_kp}
  \end{subfigure}
  \begin{subfigure}{0.5\textwidth}
    \centering
    \tikzsetnextfilename{canv_cosThL_phi_Sim08_3fb_hel_negKaons_all}
    \scalebox{0.55}{\input{Figures/Chapter4/canv_cosThL_phi_Sim08_3fb_hel_negKaons_all}}
    \caption{}
    \label{eff2D_lp}
  \end{subfigure}
...
shape an additional step is implemented. As previously mentioned, the acceptance function of \equref{eff_func_new} is
fitted to the simulated data where the efficiency moments are allowed to vary from their measured values, of \tabref{eff_moms_table}.
...
The multivariate Gaussian, $G(\vec{\bf{c}})$, used to constrain the efficiency moments is shown in \equref{mvm_constr_gauss}.
...
\item Weight each candidate in the simulated data for the difference between the angular \pdf obtained from the previous fit and the angular
      \pdf used to generate the simulated data. Impling that the underlying physics of the simulated data corresponds
...
\noindent where $\mKpiAmp[n,m]$ run the same as in \equref{ang_terms} and the phase space factor:
...
terms of \tabref{eff_moms_table}, since the $m=n$ integrals are by construction equal to one.
...
the  $\CSP$ factors which are inserted in the angular functions of \tabref{ang_distr}. Particularly, the real part
...
While the \Bs and \Bd background subtracted muon pair mass, $\mmumu$, have very similar shapes, the \mkpi spectra exhibit different shapes.
Indeed, as suggested by \figref{mkpiPlot_raw} the \Bs \mkpi \sPlot seems to be slightly distorted compared to the one of \Bd \mkpi.
...
Both of the asymmetries have to be estimated so that the experimentally measured asymmetry, $\Acp{\rm raw}$,
...
\begin{align}
\centering
  \Acp\;(\Bs \to & \jpsi \Kstarzb) = \nonumber \\
  &= \frac{\totAmp{\BsbarJpsiKst} - \totAmp{\BsJpsiKst}}{\totAmp{\BsbarJpsiKst} + \totAmp{\BsJpsiKst}},
\label{acp_corrected_amps}
\end{align}

\noindent The factor $\kappa$ of \equref{acp_corrected} accounts for the dilution of the production
asymmetry due to $\Bs-\Bsb$ oscillations \cite{LHCb-PAPER-2013-018} and is:
...
see \secref{cp_assymetries_and_total_decay_rate}.
