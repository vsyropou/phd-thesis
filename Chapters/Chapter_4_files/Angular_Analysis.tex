...
\jpsi and \Kstarzb are spin one (or vector) particles. Hence the acronym P2VV. The final state partciles are
two muons, \mmu, and a kaon-pion pair, $\kaon\pion$, coming from the decay of the \jpsi and \Kstarzb respectivelly.
...
orginate from squaring the amplitude and their exact expressions are shown in \tabref{ang_distr}.
The functions $h_n$ represent the angular dependence of each term $a_n$ term and $\mKpiAmp$ the \mkpi dependence of
the amplitude. The treatment of the \mkpi dependence is postponed for the next section. \tabref{ang_distr} lists the angular
part of the decay rate equation, \ie the $h_n$ functions, which is the result of
applying the \emph{helicity formalism} in the \BJpsiKst decay. In order to descrbe the 4-body decay \BJpsiKst, three
angles, denoted by $\Omega$, plus the \mkpi are required. As shown in \figref{helAngles}, the three \emph{heliciy angles} angles
...
Lastly, the orthogonal angular basis
...
of the product of two $P$ or two $Y$ are known analytically. The last significantly imporves the description and implementation
...
As explained in \secref{lhcb_detector} the \lhcb detector does not cover the full $4\pi$ solid angle, plus the fact that
there are no detector elemntents inside the \lhc beam pipe. The current section introduces the treatment of
...
\begin{figure}[h]
  ...
  \begin{subfigure}{0.49\textwidth}
    ...
    \scalebox{1.15}{\input{Figures/Chapter4/eff_mc_helcosthetaK_allKaons_binall}}
    \caption{}
    \label{angAcc_ctk}
  \end{subfigure}%
  \hfill%
  \begin{subfigure}{0.49\textwidth}
    ...
    \scalebox{1.15}{\input{Figures/Chapter4/eff_mc_helcosthetaL_allKaons_binall}}
    \caption{}
    \label{angAcc_ctl}
  \end{subfigure}
  ...
  \begin{subfigure}{0.49\textwidth}
    ...
    \scalebox{1.15}{\input{Figures/Chapter4/eff_mc_helphi_allKaons_binall}}
    \caption{}
    \label{angAcc_phi}
  \end{subfigure}
...
close to $\cos\thetaK=1$. The latter is mainly caussed by one of the candidate selection requirements of \tabref{Bs2JpsiKstSelection},
specifically the requirement on the \kaon and \pion transverse momentum, \pt, to be larger that $500\mevc$.
Before attempting to explain the acceptance shape it is usefull to illustrate how the decay angles are
distributed according to the \pdf of \equref{ang_terms} \ie without any acceptance effect, see \figref{angDistr_all}.

Understanding the shape of the acceptance in the $\cos\thetaK$ projection starts with the observation that
a large number of pions accumulate more towards the $\parenthesis{\pt=916\mevcc,\cos\thetaK=-1}$ phase space region
comapred to the region of $\parenthesis{\pt=916\mevcc,\cos\thetaK=+1}$. This picture is less pronounced
in the case of kaons where the $\pt$ spectrum is harder and the kaons populate the above regions of phase space more evenly.
Given the above correlations it follows that applying the \pt requirement on both \kaon and \pion makes the $\cos\thetaK$
distribution assymetric in $\cos\thetaK=\pm 1$.

\begin{figure}[t]
  \centering
  \begin{subfigure}{0.33\textwidth}
    \tikzsetnextfilename{mc_ang_ctk}
    \scalebox{0.75}{\input{Figures/Chapter4/mc_ang_ctk}}
    \caption{}
    \label{angDistr_ctk}
  \end{subfigure}%
  \hfill%
  \begin{subfigure}{0.33\textwidth}
    ...
    \scalebox{0.75}{\input{Figures/Chapter4/mc_ang_ctl}}
    \caption{}
    \label{angDistr_ctl}
  \end{subfigure}%
  \hfill%
  \begin{subfigure}{0.33\textwidth}
    ...
    \scalebox{0.75}{\input{Figures/Chapter4/mc_ang_phi}}
    \caption{}
    \label{angDistr_phi}
  \end{subfigure}
  \caption{Decay angles distributions in the simulation sample before any selection.}
  \label{angDistr_all}
\end{figure}



The above situation is related to kinematic constrains in two body decays particularly close to the physical boundaries
of the phase space. The latter constrain manifests itself when computing the angle of the \pion with respect to the boost
direction in the lab frame as shown in the following equation:

\begin{equation}
  \centering
  \tan\thetapi^\lab = \frac{\ptrans{\pi}}{\plongd{\pi}}
               = \frac{\ptrans{\pi}^\CM}{\gamma\parenthesis{\plongd{\pi}^\CM + \beta E^\CM_\pi} }
               = \frac{\ptot_{\pi}^\CM \sin\thetapi}{\gamma\parenthesis{\ptot_{\pi}^\CM \cos\thetapi + \beta E^\CM_\pi} },
  \label{pi_angle}
\end{equation}

\noindent where, $\thetapi^lab$ is the angle of the \pion momentum vector with respect to the boost direction in the lab frame.
The subscripts T,L label the transeverse and longditudinal components of the \pion momentum, \ptot. The \CM
superscript indcates quantities evaluated in the \Kstarz rest frame. The symbol $E$ stands for the energy of
the pions. The lorentz boost factor and the relativistic speed are labeled by $\gamma$ and $\beta$ repectively.
The angle $\thetapi$ is the angle of the pion momentum in \Kstarz rest frame and it is releated to $\cos\thetaK$
as: $\thetapi = \cos\thetaK + \pi$. Now, the origin of the above mentioned assymetry is understood better in the
limit of a massless pion, in which case \equref{pi_angle} reduces to:

\begin{equation}
  \centering
  \tan\thetapi = \frac{\sin\thetapi}{\gamma\parenthesis{\cos\thetapi + 1} }.
  \label{pi_angle_massless}
\end{equation}

\noindent Note that for a masless particle the energy is equal to its total momentum and also that $\beta \simeq 1$
for pions coming from \BJpsiKpi decays. It can be seen in \equref{pi_angle_massless} that decays where $\cos\thetapi=-1$
are not allowed. The later implies a low transverse momentum pion with $\cos\thetaK = -\cos\thetapi = +1$, which explains
why the shape of the $\cos\thetaK$ distribution becomes asymmetric when excuding low mementa pions. This effect is more
general and it is releated to the lightes of the two particles in a two body decay.

Coming now to the case of the $\cos\thetamu$ acceptance shape, the two decay products of the \jpsi have
the same mass. A consequence of latter is that the transeverse momentum of each of the two muons
is similarly distributed in the $\parenthesis{\ptrans{\mmu},\cos\thetamu}$, thus excluding low transverse
momentum muons does not introduce any assymetry in the $\cos\thetamu$ distribution.
...
integration. Such a parameterization is defined by \equref{eff_func}.
...
Following the above parameterizations one just needs to obtain a set of $c^i_{jk}$. Now, there are infinite efficiency moments
...
The choice of the particular efficiency moments, listed in \tabref{eff_moms_table}, is connected to an important
...
the later is the angular \pdf which the simulated data have been generated with. It follows that the efficiency is defined by \equref{eff_def}.
...
The last step in the calculation of the efficiency moments is the computation of the integral in \equref{eff_moms_def}.
...
same selection steps as real data) as shown in \equref{eff_moms_calc}.
...
\noindent Acceptance effecets in the $\cos\thetamu$ projection are less pronounced compared to the one $\cos\thetaK$.
...
moments is performed to optimize the acceptance shape discription. The result is a hybrid acceptance function that is by
construction positive definite.

Before laying out the formulas behind the above mentioned steps it is useful to visualise the problem caused by the parametrization, see \figref{angAcc_nom}
and the corresponding caption. Having done that the most natural thing to do before making any effort to force the value of \equref{eff_func}
positive at $\cos\thetaK=1$ is to make the best posible choice of efficiency moments. The moments are chosen such that the efficiency function becomes negative
at the largest posible value of $\cos\thetaK$. In other words the crossing point of the red curve with the horizontal axis in \figref{angAcc_nom},
...
Having chosen the best possibe set of moments the efficiency function has to be constrained such that the condition,
...
efficiency function is decomposed, see \equref{eff_func}, is modified as shown in \equref{eff_basis_transform}
...
The result of the above constraint is shown in figure \figref{angAcc_constr_fit}, where it can be seen that
...
to take negative values whilst the 1D projections are positive. Two dimentional projection of the constrained
...
As previously mentioned, the acceptance function of \equref{eff_func_new} is further fitted to the simulated data
where the efficiency moments are allowed to vary within one standard deviation from their measured values, of \tabref{eff_moms_table}.
...
The fitting \pdf is shown in \equref{mvm_constr} and it consists of the \pdf that the simulated
...
is shown in \equref{mvm_constr_gauss}.
...
The advantages of this method is that it is by construction immune from any issue with negative efficiency values.
...
when using normalization weights and other methods need to be implemented in order to visualise the total \pdf.
...
However, the above mentioned mapping is not unique and not always invertable. It depends on the choice of the particular
...
\begin{figure}[!t]
  ...
  \begin{subfigure}{0.49\textwidth}
    ...
    \scalebox{1.15}{\input{Figures/Chapter4/eff_mc_helcosthetaK_allKaons_binall_rew}}
    \caption{}
    \label{angAccCor_ctk}
  \end{subfigure}%
  \hfill%
  \begin{subfigure}{0.49\textwidth}
    ...
    \scalebox{1.15}{\input{Figures/Chapter4/eff_mc_helcosthetaL_allKaons_binall_rew}}
    \caption{}
  ...
  \begin{subfigure}{0.49\textwidth}
    ...
    \scalebox{1.15}{\input{Figures/Chapter4/eff_mc_helphi_allKaons_binall_rew}}
    \caption{}
...
The $p_{K^{\pm}},p_{\pi^{\mp}}$ weighting of the simulated data sample is done using 2D histograms, following the variable intervals defined in \tabref{angAccBinning}.
...
\tabref{pars_convergence}. The central values of the parameters of interest do not change significantly implyng that the overall effect of the
acceptance corrections is small. \figref{angAggCorrections} shows the shape of the acceptance before and after corrections.
This is also supported by the effect of these corrections on the paramters of intereset which are shown in \tabref{pars_convergence}.
...
\begin{tabular}{ c c c c c c c c c | c}
  ...
  \hline
\end{tabular}
\caption{Parameters of interest after each iteration. The amount of change is expresed as a fraction of one standard deviation.
         Differences get progressively smaller, impling convergence.}
\label{pars_convergence}
\end{table}

Lastly, the acceptance can be floated in the angular fit to the real data in order to assign a systematic uncertainty, see \secref{systAngAcc}.
...
Inegrals like the one of \equref{spIntegrals} eventually manifest thenselves only in the \spwave interference
terms\footnote{The diagonal $m=n$ integrals are by construction equal to 1.} of \tabref{eff_moms_table}.
Suc an intergral is in the following equation:
...
is modeled directly via the $\mKpiAmp$ terms of \equref{ang_terms}. The above mentioned line-shapes, $s$ and $p$ that are necessary
to compute the $\CSP$ factors are a combination of a \KstENT and \KstOFOZ for the \pwave, as in~\cite{PhysRevD.11.3165}.
Whereas for the \swave a LASS parametrisation~\cite{Aston1988493} is used, consisting of a linear combination of the \KstOFTZ resonance with
...
While the \Bs and \Bd sWeighted muon pair mass sWeighted, $\mmumu$, have very similar shapes, the \mkpi specta exhibit different shapes.
...
is corrected for angular acceptance effects, by weighting each event with the inverse of the acceptance function of \equref{eff_func_new}.
...
Particularly, the diffence between the number of observed \BsJpsiKst and \BsbarJpsiKst decays is releated to
the $\Acp{k}$ parameters of intereset introduced in \secref{cp_assymetries_and_total_decay_rate}.
...
of detecting the \Bs(\Bsb) final state which is a $\jpsi\Kstarzb$($\jpsi\Kstarz$), see also \secref{cp_assymetries_and_total_decay_rate}.
Both of the production and detection asymmetries have to be estimated so that the experimentally measured assymetry, $\Acp{\rm raw}$,
defined in \equref{acp_mes}, is corrected for as follows:
...
\noindent where the corrected asymmetry \Acp\;(\BsJpsiKst) is releated to the \BsJpsiKst decay
...
\noindent The factor $\kappa$ of \equref{acp_corrected} accounts for the dilution due to $B^0_s-\bar{B^0_s}$
oscillations~\cite{LHCb-PAPER-2013-018} and is spelled out in the follwoing equation:
...
average lifeteime, $\Gamma_s$, of the \BBbarSyst such that nearly all the production asymmetry is washed out by them.
...
the polariation dependant CP asymetries, \Acp{k}, as free parameters in the angular fit and how this becomes
...
split in four \mkpi intervals. This impies that for each data category, a corresponding component \pdf must be defined.
...
\BsJpsiKst. As it is mentioned in \secref{penguin_formalism} the above decays are {\it flavor specific} which means that a \Bs meson
...
Furthermore, as it can be seen in \tabref{moms_comp_signs} the agreement between the \BsJpsiKst and \BsbarJpsiKst samples
...
more details in \secref{cp_assymetries_and_total_decay_rate}.} which  significantly simplifyies the necessary computation.
...
The next observable used to split the data was already mentioned in \secref{Kpi_Invariant_mass} that dealt with the \mkpi dependence.
...
\tabref{csp_vals}, where the \CSP factors are shown for each \mkpi interval. The acceptance is also split in the \mkpi category since it was
found to vary significantly different between each \mkpi interval, see \tabref{moms_final_neg} and \tabref{moms_final_pos}.
The chosen \mkpi intervals are shown in \tabref{Kbindef}.
...
any \pdf $\mathcal{P}$ a likelihood function can be built as shown in \equref{likelihood}.
...
interest, $\vec{p}$, given the data, $\vec{x}$. The inverse of the covariance matrix for the estiamted parameters is given
...
see end of current subsection. Extending the \pdf is achieved by multiplying the likelihood with a poison distribution as shown in the fllowing equation:
...
\noindent Now, te quantities that are actually estimated in the likelihood fit are the $\AmpSq{i}$ with $i:\{0,\parallel,\perp,S\}$.
...
The fractions $\fP{k}$ and $\fS{}$ are the actual parameters that one wish to measure and correspond to those used in the subsequent anaylisis of
\chapref{Penguins}. The final expressions of the $\AmpSqPm{i}$ after all the above is given in \equref{amps_final}
...
asymmetries were estimated, the observed yields are made dependent on the $\Acp{i}$ asymmetry in \equref{acp_yields}
