...
steps of reconstruction and offline selection as the real data) and is shown in \equref{eff_moms_calc}. {\color{red} I still do not really understand the last step damm it}   
...
not guarantee that the acceptance shape discribed by \equref{eff_func} is a positive definite quantity. Note that this is an artifact of the parametrization
...
when those are beign written. The approach followed to solve this problem has two steps. First a constrain on the acceptance function \equref{eff_func} is 
...
\begin{figure}[h]
  ...
  \begin{subfigure}{0.33\textwidth}
    \scalebox{0.9}{\input{Figures/Chapter4/eff_helcosthetaK_neg_all_raw}}
    \caption{}
    \label{angAcc_nom}
  \end{subfigure}%
  \hfill%
  \begin{subfigure}{0.33\textwidth}
    \scalebox{0.9}{\input{Figures/Chapter4/eff_helcosthetaL_neg_all_raw}}
    \caption{}
    \label{angAcc_constr}
  \end{subfigure}%
  \hfill%
  \begin{subfigure}{0.33\textwidth}
    \scalebox{0.9}{\input{Figures/Chapter4/eff_helphi_neg_all_raw}}
    \caption{}
    \label{angAcc_fit}
  \end{subfigure}
  \begin{subfigure}{0.33\textwidth}
    \scalebox{0.9}{\input{Figures/Chapter4/eff_helcosthetaK_neg_all_constr}}
    \caption{}
    \label{angAcc_fit}
  \end{subfigure}%
  \hfill%
  ...
  \caption{Evolution of angular acceptance function treatment. The black points show the distribution of the helicity angles in the simulated sample 
           (This is the sample that was weighted in order to produce \figref{angAcc_ctk} to \figref{angAcc_phi}). The blue curve is the \pdf that the
...
\noindent Starting from \equref{eff_func} (where the sume now runs over the indices of the first column in \tabref{eff_moms_table}):
...
\noindent given \equref{eff_func_constr} and also $\legendre{0}{0}=1$,  $\spharm{0}{0} = \frac{1}{2\sqrt{\pi}}$ \equref{eff_func_rewriten} can be 
...
As it can be seen in \figref{} the cconstrain of the acceptance comes with a penalty. That being the fact that the shape of the 1D acceptance
...
previously mentioned the acceptance function of \equref{eff_func_final} is further fitted to the simulated data to where the efficiency
