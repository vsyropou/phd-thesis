...
\tabref{ang_distr}\footnote{It is intreasting to see in \cite{jeroen_thesis} how the $\Re$ and $\Im$ parts of the itnerference terms show up}. 
...
for the \mkpi dependence of the anplitude and its special treatment is postponed for the next section. \tabref{ang_distr} sumamrizes the angular part of the decay rate
...
\begin{figure}[h]
  ...
  \begin{subfigure}{0.5\textwidth}
    \tikzsetnextfilename{eff_helcosthetaK_neg_all_raw}
    \scalebox{1.3}{\input{Figures/Chapter4/eff_helcosthetaK_neg_all_raw}}
    \caption{}
    \label{angAcc_nom}
  \end{subfigure}%
  \hfill%
  \begin{subfigure}{0.5\textwidth}
    \tikzsetnextfilename{eff_helcosthetaK_neg_all_constr}
    \scalebox{1.3}{\input{Figures/Chapter4/eff_helcosthetaK_neg_all_constr}}
    \caption{}
    \label{angAcc_constr}
  \end{subfigure}
...
simultaneous fit over the \mkpi bins. More details on the spicifics of the fiting strategy can be found in \secref{Total_Decay_Rate}.
...
Indeed, the \Bs \mkpi \sPlot seems to be slightly distorted, \figref{mkpiPlot_raw}. Since there is now reason why the \Kst \pwave 
...
The next category that the data are split to was already mentioned in \secref{} that dealt with the \mkpi dependence. There are
...
&\text{with}\;\; \xi = \frac{\Nplus+\Nminus}{2\Nplus}
\label{acp_free}
\end{align}
...
However the quantities that are actually fitted are the $\AmpSq{i}$ with $i:\{0,\parallel,\perp,S\}$. Having said that, and by looking at \equref{acp_free}
...
as relative fractions as shown in the following equation.
...
There is one more step required to properly build the full \pdf. That is the normalization of each component \pdf. Normally the \pdfs are
normalized to the number of events corresponding in the data. But with the introduction of the \ACP asymetries in the extended likelihood 
term this is no longer possible due to the fact that the experimental assymetries as discussed in \secref{experimentalAssym} induce further
assymetries that need to be corrected for. Having already estiamted the production and detection assymetry the raw \BsbarJpsiKst and \BsJpsiKst
yields $\yieldPl[\text{raw}]$ and $\yieldMn[\text{raw}]$ respectivelly must be corrected for. 

\begin{equation}
\yield{\text{norm}}_{\pm} = \yield{CP} \left(  1 + \pm \mathcal{A}_D \mp \mathcal{A}_P\right) \frac{1}{2} 
                           \left( \fS\; \Acp[S] + \sum_k^{0,\parallel,\perp} (1-\fS)\fP[k]\Acp[k]  \right) 
\label{normyields}
\end{equation}


Explain how they are imported into the fit in the normalization of extended pdfs\\
Production and detection assymetries \\

Show likelihood function and note that oy one likelokood function is defined only. \\
