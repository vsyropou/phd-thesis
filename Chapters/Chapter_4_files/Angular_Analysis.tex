...
The functions $h_n$ and $\mKpiAmp$ represent. respectively, the angular dependence of each term $a_n$ term and the \mkpi
dependence of the amplitude. The description of the \mkpi dependence is postponed for the next section. \tabref{ang_distr}
...
\begin{figure}[t]
  ...
  \begin{subfigure}{0.33\textwidth}
    ...
    \label{angDistr_phi}
  \end{subfigure}
  \caption{Decay angles distributions in the simulation sample prior to any selection applied. Histograms are normalized to unity area.}
  \label{angDistr_all}
\end{figure}
...
It follows that the efficiency is defined by \equref{eff_def}.
...
to the {\it Kronecher delta} symbol. The later fact is required to properly normalize the efficiency moments.
...
\item Weigh each event in the simulated sample for the difference between the angular \pdf obtained from the previous fit and the angular
      \pdf used to generate the simulated data sample with. This essentially means that the underlying physics of the re-weighted simulated
      sample corresponds to the estimated physics parameters obtained in the previous step.
\item Compare the 2D $(p_\kaon,p_\pion)$ momentum distribution between the physics weighted simulated sample of the previous step with the
      data, and weigh again each simulated eventaccording to the difference.
\item Re-estimate the efficiency moments using the physics and momentum corrected \BsJpsiKst simulated sample and repeat the fit to the \BsJpsiKst data.
...
\noindent where $s$ and $p$ stand for the \swave and \pwave line-shapes respectively. Evaluating complex valued integrals yield
...
average lifetime, $\Gamma_s$. As a result the production asymmetry is nearly completely washed out by .
...
and replacing the yield fractions in intervals of \Bs momentum of Table 3 in the same reference with the backgound
subracted \BsJpsiKst ones of the current analysis, one gets the following production asymmetry:
...
There are several reasons as to why the data are devided into sub-samples.
...
The first data category is releated to the fact that the angular \pdf terms os \tabref{ang_distr} cannot describe both \BsJpsiKst
...
amplitude of \BsJpsiKst. For reasons explaine in \secref{penguin_formalism} the \BsJpsiKst decays always have a negative kaon in
...
The data categories described above result in a simultaneous fit consisting of 8 categories.
...
function is the product of probabilities of every event according to the given \pdf. Note that, contrary
to the \pdf, the likelihood is a function of the parameters given all the events in the data. It is
...
This is useful in cases where the \pdf consists of a sum over two species, such as signal and background, or if the expected number
of events depends on the parameters of interest $\vec{p}$. For the current analysis, the latter is exploited in order to
estimate the $\Acp{i}$ asymmetries in the fit. The \pdf is extended by multiplying it with the Poisson probability to
observe $n$ events when expecting $\expEvnts$ events. The expected number of events $\mu(\vec{p})$ depends on the
parameters of interest which increases the statistical uncertainties. This is beacuse the likelihood has more freadom
as the number of paramters increases and thus the shape of the likelihood becomes less steap leading to larger statistical
uncertainty.
...
explicit calculations. In addition, the above quantities can then be estimated seperately for each polarization.
...
\noindent The above yields are the expected number of events in the extended
likelihood term of the \pdfs as discussed earlier.
