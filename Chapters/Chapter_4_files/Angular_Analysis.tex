...
\noindent Note the appearance of \pwave and \spwave interference terms, in \tabref{ang_distr}; See in \cite{jeroenThesis} how the $\Re$
and $\Im$ parts of the interference terms show up. The terms $a_n$, in the same equation,
originate from squaring the amplitude and their exact expressions are shown in \tabref{ang_distr}.
...
close to $\cos\thetaK=1$. The latter is mainly caused by one of the candidate selection requirements of \tabref{Bs2JpsiKstSelection},
...
that the phase space region of $(\ptrans{\pi}-\cos\thetaK)$ is not uniformly populated. This is releated to kinematic
constrains, explained in the next paragraph. Qualitatively speaking, it turns out that \Bs candidates tend to cluster
close to the region of $(\ptrans{\pi}-\cos\thetaK=-1)$ compared to the $(\ptrans{\pi}-\cos\thetaK=+1)$ region.
This picture is less pronounced in the case of $(\ptrans{\kaon}-\cos\thetaK)$ phase space region, where the $\ptrans{\kaon}$
spectrum is harder and the $(\ptrans{\kaon}-\cos\thetaK)$ is more symmetric withg respect to $\cos\thetaK$. Given these
...
in the \Kstarz rest frame, which is releated to $\thetaK$ in \figref{helAngles} as: $\thetapi = \thetaK + \pi$.
...
the same mass and thus get the same momentum fraction out of the mother \jpsi partice. A consequence of
...
integration. Such a parametrization is defined by \equref{eff_func}.
...
Following parametrization of \equref{eff_func} one just needs to obtain a set of $c^i_{jk}$. Note that there are infinite efficiency moments
...
orthonormal basis; Implying that the integral of the product of two ascociated Legendre polynomilas is equal
to the {\it Kronecher delta} symbol. The later fact is required to properly compute the efficiency moments.
...
\begin{figure}[t]
  ...
  \begin{subfigure}{0.5\textwidth}
    ...
    \label{eff2D_lp}
  \end{subfigure}
\caption{Two dimensional efficiency projections of the \BsJpsiKst simulation sample, after the constraint of \equref{eff_func_constr}.
...
After all the acceptance shape is not parameterized at all. The biggest disadvantage on the other hand, is that directly
plotting the fitting \pdf becomes impossible when using normalization weights. This is because the normalization itegral
...
The $(p_\kaon,p_\pion)$ weighting of the simulated data sample is done using 2D histograms, following the variable intervals defined in \tabref{angAccBinning}.
...
Integrals like the one of \equref{spIntegrals} eventually manifest themselves only in the \spwave interference
...
\subsubsection{Backgound subracted \mkpi distribution}
...
While the \Bs and \Bd backgound subracted muon pair mass backgound subracted, $\mmumu$, have very similar shapes, the \mkpi spectra exhibit different shapes.
...
Particularly, the difference between the number of observed \BsJpsiKst and \BsbarJpsiKst decays is related the
main parameters of interest, introduced in \secref{cp_assymetries_and_total_decay_rate}. These parameters essentially
quantify the direct \CP asymetry between the previous decays.
...
then $\Acp{}$ will be biased by these experimental effects. In fact it happens so that both of these effects are in action.
...
while the decay time efficiency function $\varepsilon(t)$ is estimated on backgound subracted \BsJpsiKst
...
and replacing the yield fractions in intervals of \Bs momentum of Table 3 in the same reference with the backgound subracted \BsJpsiKst ones of the current analysis,
...
There are a few arguments as to why it is more convenient to devide the data sample in several sub-samples.
...
Starting from the angular \pdf terms as shown in \tabref{ang_distr}; The latter cannot describe both \BsJpsiKst and \BsbarJpsiKst.
...
\BsJpsiKst. For reasons explaine in \secref{penguin_formalism} the \BsJpsiKst decays always have a negative kaon in the final
state and \viceversa in the case of \BsbarJpsiKst decays. Thus the data can easily be split into two parts based on the sign of
the kaon. Furthermore, as it can be seen in \tabref{moms_comp_signs} the agreement between the \BsJpsiKst and \BsbarJpsiKst samples
...
more details in \secref{cp_assymetries_and_total_decay_rate}.} which  significantly simplifies the necessary computation.
...
The chosen \mkpi intervals are shown in \tabref{csp_vals}.
...
The fractions $\fP{k}$ and $\fS{}$ are the actual parameters that one wish to measure and correspond to these used in the subsequent analysis of
