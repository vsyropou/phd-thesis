...
\tabref{bestFitResult} summarizes the parameters of interest best fit values whereas
...
to \tabref{systematics_swave_phase}. Two sources of systematic uncertainties are found to be dominant. First the size of the Monte
...
\noindent where $\hat{\mu}$ denote the values of the parameters of interest, $\mu$, at the minimum and $\sigma_{\mu}$ is
...
parameter $\mu_i$ away from its value at the minimum and fixing it to that value. Subsequently likelihood
...
used to build the likelihood. Second, \equref{likelihood_exp} serves as an alternative method of estimating the statistical uncertainty
...
interval $\hat{\mu}_{-\sigma_\mu}^{+\sigma_\mu}$ is $68\%$. The previous interval is usually quoted as a $1\sigma$ {\it confidence interval}.
...
biases on the parameters of interest are estimated based on the {\it pull} variable of \equref{pull_def}.
