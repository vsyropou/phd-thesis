...
\noindent the current section. \secref{systemics} addresses systematic uncertainties.
The likelihood function is profiled against the parameters of interest in \secref{nllscans} to gain confidence on the validity
...
\figref{angPlot_thetas} show the projections of the fitted angular \pdf on the data.
Notice the desctructive interference of the \spwave. The correlation matrix of the
...
\begin{figure}[h]
  ...
  \begin{subfigure}{0.5\textwidth}
    ...
    \scalebox{1.25}{\input{Figures/Chapter4/angPlot_helcosthetaK}}
    % \caption{}
    \label{angPlot_ctk}
  \end{subfigure}%
  \hfill%
  \begin{subfigure}{0.5\textwidth}
    ...
    \scalebox{1.25}{\input{Figures/Chapter4/angPlot_helcosthetaL}}
    % \caption{}
    \label{angPlot_ctl}
  \end{subfigure}
  \begin{subfigure}{0.5\textwidth}
    ...
    \scalebox{1.25}{\input{Figures/Chapter4/angPlot_helphi}}
    % \caption{}
    \label{angPlot_phi}
  \end{subfigure}
...
The quadratic sum is assigned as systematic uncertainty.
...
results are reported in \tabref{pull_table}. The reported biases on the parameters of interest are assigned as a systematic.
...
in the same equation are uncorrelated and Gaussian in nature, then the Taylor expansion
...
\caption{Profile likelihood scans of \Acp{} parameters of interest. The black line corresponds to the profile likelihood
         function projected over the corresponding parameter. The red curve is a (symmetric) parabola.}
\end{figure}
...
\caption{Profile likelihood scans of \pwave related parameters of interest. The black line corresponds to the profile likelihood
         function projected over the corresponding parameter. The red curve is a (symmetric) parabola.
         The phases appear to have a double minimum. This is actually expected given the symmetries that the fitted \pdf
...
left and right of the minimum. The small sample size of the data categories, is depicted in the corresponding
...
\begin{figure}[!t]
  ...
  \caption{Profile likelihood scans of \swave related parameters of interest. The black line corresponds to the profile likelihood
         function projected over the corresponding parameter. The red curve is a (symmetric) parabola.
         There are a two interesting features in these plots. First $\fS{1}$ minimum is very broad due to the
...
\caption{Profile likelihood scans of \swave related parameters of interest. The black line corresponds to the profile likelihood
         function projected over the corresponding parameter. The red curve is a (symmetric) parabola.
         There are a two interesting features in these plots. First $\fS{4}$ minimum is very broad due to the
...
can indicate biases introduced to the parameters of intereset by the fitting model.
These biases are finally assigned as a systematic.
...
\begin{table}[h]
  ...
  \end{tabular}
  \caption{Pseudo-experiment study summary.}
  \label{pull_table}
\end{table}
...
The pull distributions are shown from \figref{pull_acp} to \figref{pull_swave}.
The corresponding biases are summarized in \tabref{pull_table}. Mean and rms of
each pull distribution are shown for two case. The "pull" column is computed
directly from the pull distributions, whereas the "fit" indicates the fitted
Gaussian curve on to of each pull distribution.
