...
illustrated in \figref{interference} can interfere with each other. The asymmetry associated to the interference
between mixing and decay is:
...
final state $f$ and using the \BBbarSyst decay rate formulas in \equref{decay_rate_master_eqs}, the
asymmetry of \equref{cpv_interf_def} can be expressed as:
...
to the presence of the $\sin(\Delta m_s t)$ term. The oscillation frequency is driven by $\Delta m_s$,
whereas the amplitude by $\Im\lambda_f$.
...
The imaginary part of \equref{lambda_cpv} can be expressed as a combination of CKM elements,
...
Assuming that for this final state only the leading order diagrams of \figref{bs_box}
and \figref{bs2jpsiphi} contribute to the \BBbarSyst mixing and decay respectively;
...
\cquark and \uquark quarks. Re-writing the imaginary part of \equref{lambdaf_ckm} considering \equref{ckm_angles_def} leads to:
...
\noindent Assuming tree level $\bquark \to \cquark\cquarkbar\squark$ transitions, like the one in \figref{bs2jpsiphi},
...
detector's time resolution need to be taken into account.
...
The status of the \phis measurements is shown in \figref{hfag_phis_dg}, where it can be seen that \lhcb has
the most precise measurement to date. The combined \phis value from \lhcb \cite{phis-3fb-paper}, shown in
\equref{phis_lhcb}, includes two channels \BsJpsiPhi and \BsJpsipipi:
...
As explained at the end of \secref{The_Standard_Model} Standard Model needs to be extended
...
In view of the combined \phis result illustrated in \figref{hfag_phis_dg} New Physics effects,
...
The latter points out the power of flavor physics.
...
{\it penguin} diagrams (or topologies) shown in \figref{bs2jpsiphi_peng}. The \phis measurement of
\equref{phis_lhcb} ignores the contribution of these penguin diagrams \cite{phis-3fb-paper}.
This has been a reasonable assumption before the \lhcb measurement which showed that the measured
\phiS{eff} value is consistent with the Standard Model prediction \phiS{SM,tree}, within the experimental accuracy.
...
Furthermore, the \grpsuthree flavor symmetry, is invoked since it increases the precision on the
penguin contributions estimation. The full description of the necessary formalism to determine the penguin shift, $\Delta\phiS{SM,peng}$,
is given in \chapref{Penguins}.
