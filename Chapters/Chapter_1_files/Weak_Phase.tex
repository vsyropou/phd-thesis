...
An example of the last is shown in \figref{bs2jpsiphi}. In addtion, the final state of the \Bs meson has to be
...
decay to the CP eigenstate $f$, asymmetry ascosiated to the interference can be written as shown in \equref{cpv_interf}.
...
Thus it can be seen that the assymetry of \equref{cpv_interf} shows an oscilatory behaviour versus time, see figure 4.3 in~\cite{jeroenThesis}.
...
other types of asymetries. The asymetries are shown in \equref{cp_asym_lambda} and they come from the differential decay rate equations of the $\Bs\to f$
 and $\Bsb\to f$.
...
difference in the decay rate of the mass eigenstates of \equref{bs_mass_eigen}. It is intreasting to point out that according
...
mixing even if no CP-Violation occurs in both the mixing and decay. This is posible if the imaginary part, $\Im\lambda_f$, in \equref{lambda_cpv} is not zero.
...
The imagianary part of \equref{lambda_cpv} can be expresed as a combination of CKM elements, given a certain final state $f$, like $\jpsi\phi$.
...
mixing and decay respectivelly. Under these assumptuons \equref{lambdaf_ckm} shows the leading order Standard Model estimate of $\Im(\lambda_f)$.
...
decay amplitudes respectivelly. Re-writing the imaginary part of \equref{lambdaf_ckm} taking into acount \equref{bsAnglesDef},
...
$l$ of the $\jpsi$ and $\phi$ mesons. This dependancy has to be taken into acount when going from \equref{lambdaf_ckm}
...
The parameter \phis has been measuremed at the \lhcb experiment by studing the decays of \BsJpsiPhi~\cite{phis-3fb-paper}.
...
independant\footnote{some residual dependance left, very similar to what is described in \secref{Kpi_Invariant_mass}. }
...
\begin{figure}[h]
  ...
  \caption{ Higher order-penguin diagram. Figure from~\cite{jeroenThesis}. This diagram is suppressed by the CKM matrix elements ivolved as explained in \secref{Penguins}}
  \label{bs2jpsiphi_peng}
\end{figure}

This has been a reasonable assumtion before the \lhcb measurement which showed that the measured \phiS{eff} value is consistent with the Standard
