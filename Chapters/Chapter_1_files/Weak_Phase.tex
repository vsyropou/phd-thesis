...
illustrated in \figref{interference} can interfere with eachother..
...
associated to the interference can be written as shown in \equref{cpv_interf}:

% where the parameter $\eta_f$ is the CP eigenvalue of the final state $f$ and can be either $+1$ or $-1$
% corresponding to CP-even and CP-odd respectively.
...
It can be seen that the asymmetry of \equref{cpv_interf} shows an oscillatory behaviour versus time, see figure 4.3 in~\cite{jeroenThesis}.
...
mixing even if no CP-Violation occurs in either the mixing or decay. This is possible if the imaginary part, $\Im\lambda_f$, in \equref{lambda_cpv} is not zero.
