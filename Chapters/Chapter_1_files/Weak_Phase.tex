...
There are two ways that those particles are sought for, namelly directly or inderctly. The first implies that the
assumed new particle is created in high energy physics colisions and its presentce is infered by directly detecting
the products of its decay. A typical example is the recent discovery of the higgs bosson at \lhc. In the second scenario
the presence of a new particle modifies some observable quantity in way that Standard Model cannot explain.
This way it is not necessary for this partcile to be created in the first palce.
One of the most significant examples of this type is the existance of the \bquark, which modified the fequency,
$\Delta m_d$, that the \Bd meson oscilated to its antiparticle \Bdb {\color{red} ref paper}.
The second approach is the main focus of flavor physics. According to blah and blah \phis is an a exepelent observabale for probing NP.


In view of the lhcb run1 results NP are small (e.g. bmumu p5prime, phis, Vub) thus we need to worry about next section
We got only tensions. Ntering a preceision ere, Need to overconstrain all falvor physics observables monitor them constantly

Particularly for the case of the \phis observable. We measure
...
Given the world average on \phis it is necessary to control penguins {ref robert and all}.
