...
The parameter \phis manifests in the time dependence of the so called $\bquark \rightarrow \cquark\cquarkbar\squark $ transitions.
An example of the last is shown in \figref{bs2jpsiphi}. In addition, the final state of the \Bs meson has to be
a CP eigenstate, which implies that both \Bs and \Bsb can decay into it, such that the two decay paths
of \figref{interference} can interfere.

A common way to parametrize CP violation in the interference between mixing and decay is shown in \equref{lambda_cpv}
\footnote{This $\lambda_f$ is not to be confused with the $\lambda$ of the Wolfenstein Parametrization.}.
...
corresponding to CP-even and CP-odd respectively. Assumption no CP-Violation in both \BBbarSyst mixing and in the
decay to the CP eigenstate $f$, asymmetry associated to the interference can be written as shown in \equref{cpv_interf}.
...
\noindent where $\Delta\Gamma_s$ and $\Delta m_s$ are the lifetime and mass differences of the mass eigenstates \equref{bs_mass_eigen}.
Thus it can be seen that the asymmetry of \equref{cpv_interf} shows an oscillatory behaviour versus time, see figure 4.3 in~\cite{jeroenThesis}.
The frequency of the oscillation is driven by $\Delta m_s$, whereas the amplitude by $\Im\lambda_f$.

For completion, according to ~\cite{DeBruyn-thesis,jeroenThesis}, the parameter $\lambda_f$ is also related to the $\Acp{\rm dir}$ as well as to two
...
difference in the decay rate of the mass eigenstates of \equref{bs_mass_eigen}. It is interesting to point out that according
to \equref{cpv_interf} and the \Acp{\rm dr} of \equref{cp_asym_lambda} CP asymmetry can still be observed in the interference between decay and
mixing even if no CP-Violation occurs in both the mixing and decay. This is possible if the imaginary part, $\Im\lambda_f$, in \equref{lambda_cpv} is not zero.
...
be integrated over time when measuring \phis. Implying that the time interval between the production and decay of a \Bs (\Bsb) meson has to be
...
The imaginary part of \equref{lambda_cpv} can be expressed as a combination of CKM elements, given a certain final state $f$, like $\jpsi\phi$.
Furthermore, for the last final state only the leading order diagrams of \figref{bs_box} and \figref{bs2jpsiphi} are assumed for the \Bs, \Bsb
mixing and decay respectively. Under these assumptions \equref{lambdaf_ckm} shows the leading order Standard Model estimate of $\Im(\lambda_f)$.
...
decay amplitudes respectively. Re-writing the imaginary part of \equref{lambdaf_ckm} taking into account \equref{bsAnglesDef},
one can obtain \equref{lambda_ckm_manip}
...
$l$ of the $\jpsi$ and $\phi$ mesons. This dependency has to be taken into account when going from \equref{lambdaf_ckm}
to \equref{phis_jpsiphi_def}, resulting in \equref{phis_jpsiphi_def}
...
\noindent As it was previously mentioned $\Im\lambda_{\jpsi\phi}$ is the amplitude of the time dependant asymmetry of \equref{cpv_interf}.
...
independent\footnote{some residual dependence left, very similar to what is described in \secref{Kpi_Invariant_mass}. }
of the \mkk mass variable and thus much simpler. However an angular analysis, similar to the one in \secref{Diferential_Decay_Rate} is necessary
...
The last is consistent with global fits to the Standard Model parameters \equref{phis_theo}~\cite{ckm-fitter-phis-pred}\footnote{updated with summer 2015 result from the CKM-fitter group.}
...
shown in \figref{bs2jpsiphi_peng}. The \phis measurement of \equref{phis_lhcb} ignores the contribution of the last penguin diagram.

\begin{figure}[h]
  \centering
  {\sffamily \input{Figures/Chapter1/penguin}}
  \caption{ Higher order-penguin diagram. Figure from~\cite{jeroenThesis}. This diagram is suppressed by the CKM matrix elements involved as explained in \secref{Penguins}}
  \label{bs2jpsiphi_peng}
\end{figure}

This has been a reasonable assumption before the \lhcb measurement which showed that the measured \phiS{eff} value is consistent with the Standard
Model prediction \phiS{SM,tree}, within the current experimental accuracy. However with the increasing accuracy at the \lhcb experiment the last assumption needs to be revisited.
