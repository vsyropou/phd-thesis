...
of matter and antimatter. Where, antimatter is a state of matter where the signs of all quantum numbers
of a particle, like the electric charge, are flipped. Fermions can be divided further into {\it quarks}
and {\it leptons}. The later do not interact through the {\it strong} force, which is one of the above-mentioned
...
masses for the gauge and Higgs bosons and are also contained in the same term.
...
where the mathematical notion of a group is implied. This means that there are three distinct types of transformations
that leave \equref{lagrangian} invariant. Each type of transformation introduces respectively the strong,
{\it weak} and {\it electromagnetic} interactions between the fermion fields following the concept of {\it local gauge invariance} \cite{aitchison,halzen1984quarks}.
...
The charged weak bosons \Wpm have the ability to change the flavor whereas the neutral \Z does not, see \figref{WeakInteractions}.
An important subtlety is related to the right handed anti-neutrinos, see next paragraph, for which there is no up-down type structure,
...
it follows that there are no right-handed neutrinos (and left-handed anti-neutrinos), according to the Standard Model.
This has to do with the nature of the weak force which couples only to the left-handed projection of a particle's
...
strong interactions respect these symmetries, altough the \CP conservation of the strong force is still to be fully understood.
...
However the later is not consistent with observations and nature does distinguish between matter and antimatter
through the weak interaction. The origin of \CP-violation in the Standard Model is briefly addressed in \secref{Flavour_Physics}.
