...
namely the {\it quark flavour mixing}. A brief description of the latter and its relevance to \CP-violation is given in what follows.
...
interaction, shown in \equref{CClagrangian}. The charged current interaction in the same euquation is expressed in two ways.
...
Wolfenstein parametrization utilizes three real parameters, $\lambda,A,\rho$ and an imaginary one $i\eta$, as shown in \equref{CKMwolfenstein}.
...
\noindent These two relations, after dividing by $\Vcd\Vcb^*$, $\Vcs\Vcb^*$ respectively for \Bd and \Bs, are illustrated in \figref{unitTriangles}.
...
\figref{unitTriangles_bs}. By inspecting the triangles one can define the angles as shown in \equref{ckm_angles_def}.
...
\begin{figure}[h]
  ...
  \end{subfigure}
  \caption{CKM-unitarity triangles. (left) \Bd triangle, corresponding to \equref{unitConstraints_Bd}. (right) \Bs triangle,
