In the recent \lhcb measurement \cite{phis-3fb-paper} of \phis one can found the best fit value for the $\lambda_{\jpsi\phi}$
...
Note that the information from these channels entering through the observables, \equref{bsjpsiphi_peng_acp_obs} and \equref{hobs_def}
...
This section introduces the additional channels, \BsJpsiKst and \BdJpsiRho. The details of the $\chisq$ fit are presented in \secref{penguin_chi2_fit}.
...
diagrams are illustrated in \figref{bs2jpsikst};
...
The first observables is reported in \tabref{bestFitResult}, with the associated systematic quoted in \tabref{systematics_acp}.
...
to the one described in \secref{penguin_chi2_fit}. Hence its details are postponed for later.
...
\noindent The amplitude structure is identical to that of the \BsJpsiKst mode resulting in \equref{bsjpsirho_amp}.
...
the latter is attributed to the particular value of the penguin parameter $\thkPeng{k}''$, which is 
$\sim 90^\circ$ and $\sim 10^\circ$ respectivelly for the \BdJpsiRho and \BsJpsiKst chanells. 
Specifically, as it can be seen in \equref{tandelta} the algebreic structure of the sam equation 
is such that, $\tan(\Delta\phis^k)$ becomes minimal around odd multiples of $\pi/2$.
...
the two additional channels \BsJpsiKst and \BdJpsiRho as shown in \equref{had_ratios_assuption}.
