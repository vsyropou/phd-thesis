The results of the \chisq fit to the penguin paramters, described in \secref{penguin_more_chanells}
are the following:
...
visibility. There it can be seen that the $\Hobs{k}$ observables has a smaller impact on the best fit
result due to their associated large uncertainty. Also note the tension between the $\Hobs{k}$ observables
and the best fit result, which suggests that the assumption \equref{had_ratios_assuption} is probably not true.
...
A $20\%$ $\grpsuthree_F$ breaking is assumed, according to what was mentioned at the end of
\secref{had_pars_suthree}, by performing a special fit explained in \secref{su3_breaking}.
...
it becomes mandatory to control penguin cotributions in future \phis measurments.
Furthermore, the sensitivity from the experimental side is intreasting with respect to
the Standard Model prediction, see \equref{phis_lhcb_theo}. Thus, potential deviations from these
predictions are going to play a cetral role in future and more precise \phis measuremnts.
...
followed strategy the hadronic parameter ratios are not affected by $\grpsuthree_F$ breaking since they are fitted for.
...
$\grpsuthree_F$. Specifically, the uncertainties that $\xi_k$ and $\delta_k$ are allowed to
vary within are $[0,0.5]$ and $[0,40]^\circ$ respectively.
